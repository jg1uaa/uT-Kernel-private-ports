...
#else
#if CFN_MAX_FLGID > 0 || CFN_MAX_MBFID > 0 || CFN_MAX_MPLID > 0
#define TCBSZ_WINFO	(8)
#else
#if CFN_MAX_MBXID > 0 || CFN_MAX_MPFID > 0
#define TCBSZ_WINFO	(4)
#else
#if CFN_MAX_SEMID > 0
#define TCBSZ_WINFO	(4) /* due to 4 byte align of wtmeb */
#else
...
#define _ALIGN_CPU(x)	(((x)+3)&0xFFFFFFFC)	/* H8S2212 : 32 bit align	*/


/*----------------------------------------------------------------------*/
/*	machine dependent data						*/
/*----------------------------------------------------------------------*/
#define	TCB_winfo	(56)		/* tskque - wercd		*/
